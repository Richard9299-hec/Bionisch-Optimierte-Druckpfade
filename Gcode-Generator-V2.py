...
def generate_infill_gcode(xa,ya,xb,yb,height,force):
    e = force*extrusion*height*((((float(xa)-float(xb))**2)+((float(ya)-float(yb))**2))**0.5)
...
def write_infill(lines,layerheight,avg_force,points_with_forces,x_offset):
    index_1=0
    infill=[]
    while index_1 < len(lines):
        infill.append("G0 F2700 E-2\n")
        ...
        while index_2 < len(lines[index_1]):
            ...
            midpoint = [(xb+xa)/2,(yb+ya)/2]
            if midpoint[0] < x_offset:
                offset = abs(x_offset - midpoint[0])*2
                midpoint[0] += offset
            relative_force = (get_force(midpoint,points_with_forces)/avg_force)*0.09
            relative_force = minimum(relative_force,0.14)
            forces.append(relative_force)
            infill.append(generate_infill_gcode(xa,ya,xb,yb,layerheight,relative_force+min_extrusion))
            index_2 +=1
        index_1 +=1
    return infill
...
def read_in_forces():
    points_with_force = []
    data = open("ansysdata.txt","r")
    for line in data:
        line = line.split(" ")
        line = [item.strip() for item in line if item.strip()]
        line = [float(line[0]), float(line[1]), (float(line[2])**2+float(line[3])**2)**0.5]
        points_with_force.append(line)
    return points_with_force


def move_forces(points,offset):
    for point in points:
        point[0] += offset[0]
        point[1] += offset[1]
...
def get_force(point,points_with_force):
    index = 0
    distance = dist(point,points_with_force[index][:2])
    for i,points in enumerate(points_with_force):
        if distance > dist(point,points[:2]):
            index = i
            distance = dist(point,points[:2])
    return points_with_force[index][2]

def get_max_force(points_with_force):
    max_force = 0
    for point in points_with_force:
        if point[2]>max_force:
            max_force = point[2]
    return max_force

def get_avg_force(points_with_force):
    summ = 0
    for point in points_with_force:
        summ += point[2]
    return summ/len(points_with_force)

def main():
    gcode = read_in_gcode()
    lines = read_in_lines()
    lines = mirror_lines(lines)
    offset =[get_center(gcode)[0],112.5]
    move_lines(lines,offset[0],offset[1])
    data = cut_gcode(gcode)
    forces = read_in_forces()
    move_forces(forces, offset)
    #max_force = get_max_force(forces)
    avg_force=get_avg_force(forces)
    print("Avg_force: "+ str(avg_force)+"N")
    gcode = data[1]
